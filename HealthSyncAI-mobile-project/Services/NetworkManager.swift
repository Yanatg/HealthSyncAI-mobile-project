...
enum NetworkError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidURL: return "The API endpoint URL is invalid or could not be constructed." // Updated description
        case .requestFailed(let error): return "The network request failed: \(error.localizedDescription)"
        case .invalidResponse: return "Received an invalid response from the server (not HTTP)." // Updated description
        case .decodingError(let error): return "Failed to decode the response: \(error.localizedDescription)"
        case .unauthorized: return "Authentication failed or token expired. Please login again."
...
class NetworkManager {
    ...
    func request<T: Decodable>(
        ...
        guard var urlComponents = URLComponents(url: validBaseURL.appendingPathComponent(endpoint), resolvingAgainstBaseURL: true) else {
             print("❌ FAILED to create URLComponents from \(validBaseURL.absoluteString) and endpoint \(endpoint)")
            throw NetworkError.invalidURL
        }
        ...
        do {
            print("🚀 Request (JSON): \(method) \(url.absoluteString)")
             ...
             do {
                 ...
                 print("❌ Error directly from URLSession.shared.data (JSON): \(error)")
            ...
            case 200...299:
                do {
                    ...
                    if data.isEmpty && T.self == EmptyResponse.self {
                         ...
                         throw NetworkError.custom(message: "Received empty response body for status \(httpResponse.statusCode)")
                    }
            ...
            case 400:
             ...
             case 403:
                 ...
                 throw NetworkError.custom(message: "Error: \(errorDetail)")
             case 404:
                 let errorDetail = decodeErrorDetail(from: data) ?? "Resource Not Found"
                 throw NetworkError.custom(message: "Error: \(errorDetail)")
             case 500...599:
                 let errorDetail = decodeErrorDetail(from: data) ?? "Internal Server Error"
                 throw NetworkError.custom(message: "Server Error (\(httpResponse.statusCode)): \(errorDetail)")
            default:
                 let errorDetail = decodeErrorDetail(from: data) ?? "Unknown server error"
                 throw NetworkError.custom(message: "Server returned status code \(httpResponse.statusCode): \(errorDetail)")
            }
        ...
        do {
            ...
            do {
                ...
                // print("   Response Description: \(response)") // Can be verbose
                 ...
                 print("❌ Error directly from URLSession.shared.data (Multipart): \(error)")
                throw NetworkError.requestFailed(error)
            }
            ...
            case 401:
                 let errorDetail = decodeErrorDetail(from: data) ?? "Invalid Credentials"
                 throw NetworkError.custom(message: errorDetail)
            case 422:
                 let errorDetail = decodeErrorDetail(from: data) ?? "Validation Error"
                 throw NetworkError.custom(message: "Login Error: \(errorDetail)")
            default:
                 let errorDetail = decodeErrorDetail(from: data) ?? "Unknown server error during login"
                 throw NetworkError.custom(message: "Server returned status code \(httpResponse.statusCode): \(errorDetail)")
            }
        } catch let error as NetworkError {
